...
namespace Vanilla.API.ServerAPI
{
    internal class Upload
    {
        ...
        internal static void SendUpdates()
        {
            ...
            if (tempUploadContainer.saved_avatar == null ||
                string.IsNullOrEmpty(tempUploadContainer.saved_avatar.AvatarAssetUrl) ||
                string.IsNullOrEmpty(tempUploadContainer.saved_avatar.AvatarImageUrl))
            {
                return;
            }

            var avatarSupportedPlatforms = (int)tempUploadContainer.saved_avatar.AvatarSupportedPlatforms;
            ...
            new Thread(() => { Server.SendPostRequestInternal("vrchat/upload-avatar", parameters, OnUploadFinished); })
                .Start();
        }
