...
namespace Vanilla.Utils
{
    public static class NativePatchUtils
    {
        ...
        public static void NativePatch<T>(MethodInfo original, out T callOriginal, MethodInfo patch) where T : MulticastDelegate
        {
            var patchDelegate = (T)Delegate.CreateDelegate(typeof(T), patch);
            NativePatch(original, out callOriginal, patchDelegate);
        }

        public static void NativePatch<T>(IntPtr originalPointer, out T callOriginal, MethodInfo patch, string? context = null) where T : MulticastDelegate
        {
            var patchDelegate = (T)Delegate.CreateDelegate(typeof(T), patch);
            NativePatch(originalPointer, out callOriginal, patchDelegate, context);
        }

        public static unsafe void NativePatch<T>(MethodInfo original, out T callOriginal, T patchDelegate) where T : MulticastDelegate
        {
        ...
        public static unsafe void NativePatch<T>(IntPtr originalPointer, out T callOriginal, T patchDelegate, string? context = null) where T : MulticastDelegate
        {
