...
namespace Vanilla.Utils
{
    ...
    {
#if DEBUG
        protected internal static readonly bool DevMode = true;
#else
        ...
        private static readonly ConcurrentQueue<ConsoleLog> WrittenToConsole = new();

        private static readonly MelonLogger.Instance loggerInstance = new("AbandonWare");
        private static readonly MelonLogger.Instance CypherEngineLogger = new("CypherEngine");

        internal static void LogToHud(string message)
        {
            OnScreenUI.AddString(string.Format("<color=#ff0000> {0} </color>", message.ToString()));
        }

        internal static void CypherEngineLog(string Identify, object message, ConsoleColor color = ConsoleColor.White,
            string caller = null)
        {
            CypherEngineLogger.Msg(color, "[" + Identify + "] " + message);
        }

        internal static void HudLog(string Identify, object message, ConsoleColor ConsoleColor = ConsoleColor.White,
            string caller = null)
        {
        ...
        internal static void Log(string identify, object message, ConsoleColor color = ConsoleColor.White,
            string caller = null)
        {
            if (message is null)
            {
        ...
        internal static void Error(string identify, object message, ConsoleColor color = ConsoleColor.Red,
            string caller = null)
        {
            if (message is null)
            {
        ...
        {
            for (var i = 0; i < WrittenToConsole.Count; i++)
            {
                ...
                var num = result.text.ToString().LastIndexOf("<color=");
                if (num != -1)
                {
                    var num2 = result.text.ToString().IndexOf('>', num);
                    var htmlString = result.text.ToString().Substring(num + 7, num2 - (num + 7));
                    if (ColorUtility.TryParseHtmlString(htmlString, out var color))
                    {
                        ...
                        {
                            loggerInstance.Msg(color.ClosestConsoleColor(),
                                "[" + result.identifier + "]: " + result.text.StripHtml());
                        }
                        else
                        {
                            loggerInstance.Msg(color.ClosestConsoleColor(),
                                "[" + result.identifier + "] [" + result.callerName + "] " + result.text.StripHtml());
                        }
                    }
                    else
                    {
                        if (result.callerName == null)
                        {
                            loggerInstance.Msg(result.textColor,
                                "[" + result.identifier + "]: " + result.text.StripHtml());
                        }
                        else
                        {
                ...
                if (result.LogToHud)
                {
                    if (string.IsNullOrEmpty(result.text.ToString()))
                    {
                        return;
                    }

                    OnScreenUI.AddString(
                        $"[{result.identifier}] <color={result.textColor.ToHex()}> {result.text} </color>");
        ...
        internal static void RePop()
        {
            for (var i = 0; i < WrittenToConsole.Count; i++)
            {
                ...
                {
                    loggerInstance.Msg(result.textColor,
                        "[" + result.identifier + "] [" + result.callerName + "] " + result.text);
                }
