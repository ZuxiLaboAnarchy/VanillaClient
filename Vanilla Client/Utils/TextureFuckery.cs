...
namespace Vanilla.Utils
{
    ...
    {
        private static Dictionary<Texture2D, Texture2D> cachedReadableTexture2D = new();
        private static Dictionary<Texture, Texture2D> cachedConvertedTexture2D = new();
        private static Dictionary<Texture2D, Texture2D> cachedDesaturatedTexture2D = new();
        private static Dictionary<Sprite, Texture2D> cachedUnpackedTexture2D = new();
        ...
        {
            if (cachedConvertedTexture2D.Any(a => a.Key != null && a.Key == texture))
            {
                return cachedConvertedTexture2D.FirstOrDefault(a => a.Key != null && a.Key == texture).Value;
            }

            var currentFilter = texture.filterMode;
        ...
        {
            if (cachedReadableTexture2D.Any(a => a.Key != null && a.Key == texture))
            {
                return cachedReadableTexture2D.FirstOrDefault(a => a.Key != null && a.Key == texture).Value;
            }
        ...
        {
            if (cachedDesaturatedTexture2D.Any(a => a.Key != null && a.Key == oldText))
            {
                return cachedDesaturatedTexture2D.FirstOrDefault(a => a.Key != null && a.Key == oldText).Value;
            }

            var tempTexture = oldText.isReadable ? oldText : oldText.CloneReadable();
            tempTexture.hideFlags |= HideFlags.DontUnloadUnusedAsset;
            var greyTexture = new Texture2D(oldText.width, oldText.height);
            greyTexture.hideFlags |= HideFlags.DontUnloadUnusedAsset;
            Color[] c = tempTexture.GetPixels();
            var cDesat = new Color[c.Length];

            for (var i = 0; i < c.Length; i++)
            {
                var desat = (c[i].r + c[i].g + c[i].b) / 3;
                cDesat[i].r = desat;
                cDesat[i].g = desat;
                cDesat[i].b = desat;
                cDesat[i].a = c[i].a;
        ...
        {
            if (cachedUnpackedTexture2D.Any(a => a.Key != null && a.Key == sprite))
            {
                return cachedUnpackedTexture2D.FirstOrDefault(a => a.Key != null && a.Key == sprite).Value;
            }
            ...
            else
            {
                ...
                foreach (var vector in sprite.uv)
                {
                    if (vector.x > xMax)
                    {
                        xMax = vector.x;
            ...
            var readableText = sprite.texture.isReadable ? sprite.texture : sprite.texture.CloneReadable();
            Color[] c = readableText.GetPixels((int)actualRect.x, (int)actualRect.y, (int)actualRect.width,
        ...
        public static Sprite ReplaceTexture(this Sprite sprite, Texture2D newTexture)
        {
            ...
            var newSprite = Sprite.CreateSprite_Injected(newTexture, ref rect, ref pivot, sprite.pixelsPerUnit, 0,
